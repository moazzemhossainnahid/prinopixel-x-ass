    ...
    return (
        <div className='w-full relative pt-40'>
            <div className={`bg-[#FFB300] w-96 h-52 px-20 py-7 ${isSticky ? 'sticky top-0 z-20 bg-transparent' : 'sticky top-5'}`}>
                <h3 className={`uppercase tracking-widest text-xl font-semibold ${isSticky ? 'text-white z-20' : 'text-black z-20'}`}> <div className={`h-2 w-20 rounded-full bg-white absolute top-10 -left-14 ${isSticky ? 'block' : 'hidden'}`}></div> X Timeline</h3>
            </div>
            <div className="relative -mt-20 z-0">
                <div data-aos="fade-up" className="bg-black bg-inherit h-screen w-full bg-cover bg-center">
